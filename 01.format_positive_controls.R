...
#write.csv(merged_positive_gene_list,file = "/Users/dandantan/Desktop/IntAct_spark_dataset/merged_positive_control_gene_list.csv", row.names = FALSE)

########################################################################
########### Initialize an empty data frame
# gps_with_exwas <- data.frame(
#   gene = character(),
#   ensmble_gene_gps=character(),
#   trait_exwas_ei = character(),
#   trait_gps = character(),
#   category_gps = character(),
#   stringsAsFactors = FALSE
# )
# 
# # Loop through each row in ref_genes_GPS
# for (i in 1:nrow(ref_genes_GPS)) {
#   # Get the current gene
#   current_gene <- ref_genes_GPS$Gene[i]
#   
#   # Find all matching rows in merge_exwas_ei
#   matched_rows <- merge_exwas_ei[merge_exwas_ei$hgnc_gene_name.x == current_gene, ]
#   
#   # If there are matches, add each one to gps_with_exwas
#   if (nrow(matched_rows) > 0) {
#     for (j in 1:nrow(matched_rows)) {
#       gps_with_exwas <- rbind(gps_with_exwas, data.frame(
#         gene = current_gene,
#         ensmble_gene_gps=ref_genes_GPS$ensmble_genes[i],
#         trait_exwas_ei = matched_rows$Trait.x[j],
#         trait_gps = ref_genes_GPS$Phecode.description[i],
#         category_gps = ref_genes_GPS$Category[i],
#         stringsAsFactors = FALSE
#       ))
#     }
#   } else {
#     # If no matches, add a row with NA for trait_exwas_ei
#     gps_with_exwas <- rbind(gps_with_exwas, data.frame(
#       gene = current_gene,
#       ensmble_gene_gps=ref_genes_GPS$ensmble_genes[i],
#       trait_exwas_ei = NA,
...
#   }
# }
# gps_with_exwas$gene_trait_pair_exwas<- paste(gps_with_exwas$gene, gps_with_exwas$trait_exwas_ei, sep = "_")
# gps_with_exwas$gene_trait_pair_gps<- paste(gps_with_exwas$gene, gps_with_exwas$trait_gps, sep = "_")
# 
# #write.csv(gps_with_exwas,file = "/Users/dandantan/Desktop/IntAct_spark_dataset/merged_positive_control_gene_list_raw.csv", row.names = FALSE)
